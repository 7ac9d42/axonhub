...
func TestCopilotHandlers_Exchange_StateDeletedOnTokenExchangeFailure(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.URL.String() == copilot.TokenURL {
	...
	router.POST("/admin/copilot/oauth/start", h.StartOAuth)
	router.POST("/admin/copilot/oauth/exchange", h.Exchange)
	...
	exchangeBody, err := json.Marshal(ExchangeCopilotOAuthRequest{
		SessionID:   startResp.SessionID,
		CallbackURL: "http://localhost:1455/auth/callback?code=test-code&state=" + startResp.SessionID,
	})
	require.NoError(t, err)

	exchangeReq := httptest.NewRequest(http.MethodPost, "/admin/copilot/oauth/exchange", bytes.NewBuffer(exchangeBody))
	...
	exchangeReq2 := httptest.NewRequest(http.MethodPost, "/admin/copilot/oauth/exchange", bytes.NewBuffer(exchangeBody))
...
func TestCopilotHandlers_Exchange_RejectsStateMismatch(t *testing.T) {
	gin.SetMode(gin.TestMode)

	tokenServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"access_token":"a","token_type":"bearer","scope":"read:user"}`))
	}))
	t.Cleanup(tokenServer.Close)
